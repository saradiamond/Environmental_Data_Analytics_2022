...
print(O3plot) #default background

O3plot1 <- ggplot(EPAair) +
  geom_point(aes(x = Date, y = Ozone)) +
  theme_gray()
print(O3plot1) #using the default also 
...
O3plot3 <- ggplot(EPAair) +
  geom_point(aes(x = Date, y = Ozone)) +
  theme_light() #grey lines
print(O3plot3)

O3plot4 <- ggplot(EPAair) +
  geom_point(aes(x = Date, y = Ozone)) +
  theme_classic()
print(O3plot4) #no gridlines 
```
...
#defining our own theme based on classic one, increase the font, change text elements, color - black, changing the legend position
...
theme_set(mytheme) #defining theme from now own
...
display.brewer.all(n = 9) #all the palettle names
