...
ggplot(EPAair, aes(x = Date, y = Ozone)) + 
  geom_point() #to get a point

O3plot <- ggplot(EPAair) +
  geom_point(aes(x = Date, y = Ozone)) #aes can be on the first line or seconf
print(O3plot) #good if you want more than one plot

# Fix this code
O3plot2 <- ggplot(EPAair) +
  #geom_point(aes(x = Date, y = Ozone, color = "blue"))
  geom_point(aes(x = Date, y = Ozone), color = "blue") #color has to be outside aes command
print(O3plot2)
...
PMplot <- 
  ...
  geom_point()
print(PMplot) #specfied shape according to each year, colors according to site (defaults)

# Separate plot with facets
PMplot.faceted <-
  ggplot(EPAair, aes(x = Month, y = PM2.5, shape = as.factor(Year))) +
  geom_point() +
  facet_wrap(vars(Site.Name), nrow = 3) #removed color but the facet wrap will be based on the site name (now you will see a different graph for each site instead of diff color)
print(PMplot.faceted)

# Filter dataset within plot building and facet by multiple variables 
#just want to see some sites not all by using subset function
PMplot.faceted2 <- 
  ggplot(subset(EPAair, Site.Name == "Clemmons Middle" | Site.Name == "Leggett" |
                  Site.Name == "Bryson City"), 
         aes(x = Month, y = PM2.5)) + 
  geom_point() +
  facet_grid(Site.Name ~ Year) #showing the three sites in each of the two years 
...
PMplot.line <- 
  ggplot(subset(EPAair, Site.Name == "Leggett"), 
         aes(x = Date, y = PM2.5)) +
  geom_line()
print(PMplot.line) #using line to show time series have to change x to dates so they connect
...
lightvsDO <- 
  ...
  geom_point()
print(lightvsDO) #visualizing relationships between variables
#doesnt look good 
...
lightvsDOfixed <- 
  ...
  geom_point() +
  xlim(0, 250) +
  ylim(0, 20) #adjust y and x to zoom in where the points are most concentrated
...
tempvsdepth <- 
  ...
  geom_point() +
  scale_y_reverse() #reverse bc depth starts at 0 and goes deep down further
print(tempvsdepth)

NvsP <-
  ggplot(PeterPaul.chem.nutrients, aes(x = tp_ug, y = tn_ug, color = depth)) +
  geom_point() +
  geom_smooth(method = lm) + #finding the trend line for lm 
...
Nutrientplot3 <-
  ...
  geom_boxplot(aes(color = nutrient)) # Why didn't we use "fill"? #color is characterized by nutrient 
#
print(Nutrientplot3) #use color on boxplots instead of fill
...
#filling the contour of dots and colors binaxis =y means youre stacking observations over y, binwidth changes width, stackdir = how are we stacking the point, dodge means no overlaying of the dots

# Violin plot
Nutrientplot5 <-
  ggplot(PeterPaul.chem.nutrients.gathered, aes(x = lakename, y = concentration)) +
  geom_violin(aes(color = nutrient)) #
print(Nutrientplot5) #density plot

# Frequency polygons
# Using a tidy dataset
#theres no legend which isnt good
Nutrientplot6 <-
...
#this is the easier way to do plot above
Nutrientplot7 <-   
...
print(Nutrientplot6) #filling so you have diff colors for each lake
