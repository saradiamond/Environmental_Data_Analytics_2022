...
```{r, fig.height = 3, fig.width = 4}
#histogram of discharge mean column
...
#scale make its go from 0-500 on x axis
ggplot(USGS.flow.data, aes(x = discharge.mean)) +
  geom_histogram(binwidth = 10) + 
  scale_x_continuous(limits = c(0, 500))
  
#looking at gage height mean
ggplot(USGS.flow.data) +
...
```{r, fig.height = 3, fig.width = 4}
#plotting all three parameters, lty = 2 means dotted line
ggplot(USGS.flow.data) +
...
#color denotes which are A and P and we also added a legend
ggplot(USGS.flow.data) +
...
```{r, fig.height = 3, fig.width = 4}
#looking at the height mean of each approval type, shows the distreibution of values, line is the median and the box is the quantiles 1.5xiqr - 50% of values are in box, the ones outside are outliers
ggplot(USGS.flow.data) +
  geom_boxplot(aes(x = gage.height.mean.approval, y = gage.height.mean))

#group is taking slices of the data, cut_width showing the distirbution of values in gage height mean
ggplot(USGS.flow.data) +
  geom_boxplot(aes(x = gage.height.mean, y = discharge.mean, group = cut_width(gage.height.mean, 1)))

#range of values and the distirbution within that range, a lot of data close to the median
ggplot(USGS.flow.data) +
...
```{r, fig.height = 3, fig.width = 4}
#looks at relationships among variables, as we get more volume gage heught does not increase as much
ggplot(USGS.flow.data) +
...
bar plot = categorical, histogram = numeric, frequency = continuous data, box = continuous numerical data, violin = continuous numerical data, scatter = shows relationships between continuous data rather than the distributions
