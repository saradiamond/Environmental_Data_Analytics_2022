...
Rainfall_ts_decomp <- stl(Rainfall_ts, s.window = "periodic") #decomposing each ts
Temo_ts_decomp <- stl(Temp_ts, s.window = "periodic")

plot(Rainfall_ts_decomp)
plot(Temo_ts_decomp) #visualizing the time series
...
Rainfall.components <- as.data.frame(Rainfall_ts_decomp$time.series[,1:3])
Temp.components <- as.data.frame(Temo_ts_decomp$time.series[,1:3])

# Exercise 10: Visualize how the trend maps onto the data for both series

Rainfall_components <- mutate(Rainfall.components,
        Observed = BRA_complete$Rainfall,     
        Date = BRA_complete$Date)

Temp_components <- mutate(Temp.components,
