...
```{r}
#Exercise 2:
Ozone.plot <- ggplot(EPAair, aes(x = Ozone, fill=as.factor(Year))) +
  #geom_density(stat = "count", fill = "gray") +
  geom_density(alpha = 0.5) +
  scale_x_continuous(expand = c(0, 0)) + scale_y_continuous(expand = c(0, 0)) +
  labs(fill = "Year", title = "Density Plot: Ozone by Year", x = "Ozone AQI Value", 
       y = "Density") +
  scale_fill_manual(values=c("blue", "yellow"))
print(Ozone.plot)

#boxplot
Box.Ozone <- ggplot(EPAair, aes(x= as.factor(Year), y = Ozone)) +
  geom_boxplot() +
...
library(corrplot)
the_cor <- cor(EPAair %>% select(SITE_LATITUDE:PM2.5) %>% drop_na())

corrplot(the_cor, method = "ellipse")
...
the_lm6 <- lm(data = EPAair, PM2.5 ~ Ozone + Year + Month + SITE_LATITUDE + SITE_LONGITUDE)
summary(the_lm6)
