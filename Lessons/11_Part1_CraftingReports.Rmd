...
dataset.name <- c("EPA Air", "NEON NWIO Litter", "NTL LTER Lake Chem", "NTL LTER Lake Nutrients", "Ozone", "Wind Speed")

dataset.info <- c("PM25 and O3 information", "different litter information", "lake chemistry info", "lake nutrients info", "time series ozone", "data regarding wind speeds")

dataframe <- data.frame(dataset.name, dataset.info)

knitr::kable(dataframe, caption = "Table with Datasets and Information", col.names = c("Datasets", "Information"))
...
NTL.LTER.TotalNit <- NTL.LTER.Wrangled %>% 
  select(lakename, tn_ug) %>% 
  group_by(lakename) %>% 
  summarise(mean_tn = mean(NTL.LTER.Wrangled$tn_ug),
            max_tn = max(NTL.LTER.Wrangled$tn_ug),
            min_tn = min(NTL.LTER.Wrangled$tn_ug),
            sd_tn = sd(NTL.LTER.Wrangled$tn_ug))

#total phosphorus dataset
NTL.LTER.TotalPhos <- NTL.LTER.Wrangled %>% 
  select(lakename, tp_ug) %>% 
  group_by(lakename) %>% 
  summarise(mean_tp = mean(NTL.LTER.Wrangled$tp_ug),
            max_tp = max(NTL.LTER.Wrangled$tp_ug),
            min_tp = min(NTL.LTER.Wrangled$tp_ug),
            sd_tp = sd(NTL.LTER.Wrangled$tp_ug))
```
...
ggplot(data = NTL.LTER.Wrangled, aes(x = sampledate, y = tn_ug, color = lakename)) +
    geom_line() +
...
ggplot(data = NTL.LTER.Wrangled, aes(x = sampledate, y = tp_ug, color = lakename)) +
    geom_line() +
    scale_color_brewer(palette = "YlOrRd") +
