...
`>`
`>=`
`&`multiple conditions to both be true
`|`

```{r}
class(NTL.phys.data$lakeid) #can only compare factor w factor ot number w number
...
NTL.phys.data.surface3 <- filter(NTL.phys.data, depth < 0.25) #they both have the same number of observations
...
NTL.phys.data.PeterPaul1 <- filter(NTL.phys.data, lakename == "Paul Lake" | lakename == "Peter Lake") #just the two lakes 
NTL.phys.data.PeterPaul2 <- filter(NTL.phys.data, lakename != "Central Long Lake" & 
                                     ...
                                     lakename != "Ward Lake" & lakename != "West Long Lake") #you can also do it by saying the lake name is not the other ones
NTL.phys.data.PeterPaul3 <- filter(NTL.phys.data, lakename %in% c("Paul Lake", "Peter Lake")) #just a cleaner way to get the same result

# Choose a range of conditions of a numeric or integer variable
summary(NTL.phys.data$daynum)
NTL.phys.data.JunethruOctober1 <- filter(NTL.phys.data, daynum > 151 & daynum < 305) #range of observations
NTL.phys.data.JunethruOctober2 <- filter(NTL.phys.data, daynum > 151, daynum < 305)#can also just use the comma instead of &
...
NTL.phys.data.Tuesday.year <- filter(NTL.phys.data, lakename == "Tuesday Lake" & year4 >= 1990 & year4 <=1999)
...
```{r}
NTL.phys.data.depth.ascending <- arrange(NTL.phys.data, depth) #always default to ascending order
...
#just taking the columns that are relevant for you 
...
NTL.phys.data.temps <- mutate(NTL.phys.data.temps, temperature_F = (temperature_C*9/5) + 32) #adding a new column with farenheight
...
NTL.phys.data.PeterPaul1 <- mutate(NTL.phys.data.PeterPaul1, month = month(sampledate)) #taking the month from the date column

# reorder columns to put month with the rest of the date variables
NTL.phys.data.PeterPaul1 <- select(NTL.phys.data.PeterPaul1, lakeid:daynum, month, sampledate:comments) #select can be used to move column order

# find out the start and end dates of the dataset
interval(NTL.phys.data.PeterPaul1$sampledate[1], NTL.phys.data.PeterPaul1$sampledate[21613]) #first and last rows (check dims of data to get)
interval(first(NTL.phys.data.PeterPaul1$sampledate), last(NTL.phys.data.PeterPaul1$sampledate)) #finds the first and last date and easier
```
...
NTL.phys.data.processed <- 
  ...
  mutate(temperature_F = (temperature_C*9/5) + 32) #easier to do functions like this example
...
```{r}
write.csv(NTL.phys.data.PeterPaul1, row.names = FALSE, file = "./Data/Processed/NTL-LTER_Lake_ChemistryPhysics_PeterPaul_Processed.csv") #this is how you save a data set
```
